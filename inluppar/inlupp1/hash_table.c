...
struct hash_table {
  // entry_t buckets[INITIAL_CAPACITY];
  entry_t *buckets;
  size_t size;
  size_t capacity;
...
static unsigned get_bucket_index(ioopm_hash_table_t *ht,
                                 ioopm_hash_function hash_fun, elem_t key) {
...
static void resize(ioopm_hash_table_t *ht, size_t new_capacity) {
  ...
  for (size_t i = 0; i < ht->capacity; ++i) {
    ...
    while (current != NULL) {
      ...
      entry_t *new_entry = entry_create(current->key, current->value,
                                        new_buckets[new_index].next);
...
void ioopm_hash_table_insert(ioopm_hash_table_t *ht, elem_t key, elem_t value) {
  ...
  entry_t *entry =
      find_previous_entry_for_key(&ht->buckets[bucket_index], key, ht->eq_fun);
  entry_t *next = entry->next;
  ...
  if (next == NULL) {
    ...
    // if (ht->size >= ht->capacity * BUCKET_THRESHOLD) {
    // if ((double)ht->size / ht->capacity > BUCKET_THRESHOLD) {
    //   size_t new_capacity = ht->capacity * 2;
    //   resize(ht, new_capacity);
    // }
  } else {
...
bool ioopm_hash_table_has_value(ioopm_hash_table_t *ht, elem_t value) {
  for (int i = 0; i < ht->capacity; i++) {
    ...
    while (current != NULL) {
      ...
      if (!strcmp(current->value.string, value.string) &&
          !strcmp(duplicate, value.string) &&
...
void ioopm_hash_table_apply_to_all(ioopm_hash_table_t *ht,
                                   ioopm_apply_function apply_fun, void *arg) {
  for (int i = 0; i < ht->capacity; i++) {
    entry_t *current = (&ht->buckets[i])->next;

    while (current != NULL) {
      apply_fun(current->key, &current->value,
                arg); // address of value to apply function
      current = current->next;
    }
  }
}
